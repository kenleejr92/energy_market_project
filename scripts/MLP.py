...
class MLP(object):
    def __init__(self, log_difference=True):
    ...
    def predict(self, time_series):
        ...
        if self.log_difference == True:
            y_pred = np.exp(y_pred + self.test_past[self.look_back + 1:])
            y = np.exp(y + self.test_past[self.look_back + 1:])
            y_pred = self.scaler.inverse_transform(y_pred)
            y = self.scaler.inverse_transform(y)
        print y_pred.shape
        mae = np.mean(np.abs(y_pred-y))
        trivial = np.mean(np.abs(y[1:] - y[:-1]))
        print 'MAE:', mae
