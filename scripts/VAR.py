...
class VAR(object):
    ...
    def predict(self, time_series):
        ...
        X = X.reshape((X.shape[0], self.look_back*X.shape[2]))
        y = np.array(y)
        y_pred = self.vector_regressor.predict(X)
        return y_pred, y
...
if __name__ == '__main__':
    ...
    var = VAR(log_difference=True, forecast_horizon=1)
    var.train(train, look_back=48)
    predicted, actual = var.predict(test)
    var.print_statistics(predicted, actual)
    var.plot_predicted_vs_actual(predicted, actual)
